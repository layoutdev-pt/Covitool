    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <div className="bg-white p-10 rounded-[40px] shadow-xl border border-gray-100 w-full max-w-md text-center relative overflow-hidden">
          ...
          <div className="absolute top-0 right-0 w-40 h-40 bg-[#B5D318] rounded-full blur-[80px] opacity-20 -translate-y-1/2 translate-x-1/2"></div>
          
          <div className="w-20 h-20 bg-[#153A81] rounded-full flex items-center justify-center mx-auto mb-6 relative z-10 shadow-lg">
            <span className="material-symbols-outlined text-[#B5D318] text-4xl">lock</span>
          </div>
          <h2 className="text-3xl font-black text-[#153A81] mb-2 relative z-10 tracking-tight">Acesso Restrito</h2>
          <p className="text-gray-500 mb-8 font-medium relative z-10">Área de gestão do website Covitool.</p>
          ...
          <form onSubmit={handleLogin} className="flex flex-col gap-4 relative z-10">
            <input 
              ...
              className={`p-4 border-2 rounded-2xl outline-none transition-all text-center tracking-widest font-bold ${
                error ? 'border-red-400 focus:border-red-500 bg-red-50' : 'border-gray-200 focus:border-[#B5D318] bg-gray-50'
              }`}
            />
            {error && <p className="text-red-500 text-sm font-semibold">Palavra-passe incorreta.</p>}
            
            <button type="submit" className="w-full bg-[#B5D318] hover:bg-[#a1bc12] text-[#153A81] font-black py-4 rounded-2xl transition-all shadow-md hover:-translate-y-1 mt-2">
              Entrar no Painel
            </button>
            <Link to="/" className="text-gray-400 hover:text-[#153A81] text-sm mt-4 transition-colors font-semibold flex items-center justify-center gap-1">
    ...
    <div className="flex h-screen bg-gray-50 font-sans overflow-hidden">
      ...
      <aside className="w-64 bg-white border-r border-gray-200 flex flex-col shadow-sm z-20">
        <div className="p-6 border-b border-gray-100 flex items-center gap-3">
          <div className="w-8 h-8 bg-[#153A81] rounded-lg flex items-center justify-center">
            <span className="material-symbols-outlined text-[#B5D318] text-lg">settings</span>
          </div>
      ...
      <main className="flex-1 overflow-y-auto bg-gray-50 relative">
        <header className="bg-white/80 backdrop-blur-md border-b border-gray-200 p-6 flex justify-between items-center sticky top-0 z-10">
          <h2 className="text-xl font-bold text-[#153A81]">Gestão de Conteúdo</h2>
          <div className="flex items-center gap-3">
            <span className="text-sm font-semibold text-gray-500">Modo Administrador</span>
            <div className="w-10 h-10 bg-[#B5D318] text-[#153A81] rounded-full flex items-center justify-center font-black shadow-sm">
              AD
            </div>
          </div>
        </header>
