    ...
    <section className="bg-[#f8f9fa] py-24 overflow-hidden">
      ...
      <div className="container mx-auto px-4 max-w-7xl">
        ...
        <div className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-4 select-none">
          <div>
            <span className="text-brand-blue text-xs font-black tracking-widest uppercase mb-2 block">
              Destaques
            </span>
          ...
          {/* Dica visual para usar o carrossel corrigida para Mobile e Desktop */}
          {produtos.length > 1 && (
            <div
              className={`items-center gap-2 text-gray-400 bg-white px-4 py-2 rounded-full shadow-sm border border-gray-100 w-fit ${produtos.length > 3 ? "flex" : "flex md:hidden"}`}
            >
        ...
        {loading ? (
          ...
          <div className="text-center py-16 bg-white rounded-3xl border border-gray-100">
            ...
            <p className="text-gray-500 mt-2">
              Os produtos em evidência aparecerão aqui.
            </p>
          </div>
        ) : (
          ...
          >
            {produtos.map((produto) => {
              ...
              return (
                <div
                  key={produto.id}
                  className="w-[85vw] sm:w-[320px] md:w-[380px] snap-start shrink-0 bg-white rounded-[32px] p-6 shadow-sm hover:shadow-xl transition-all duration-300 border border-gray-100 flex flex-col group"
                >
                  ...
                  <div
                    className={`flex flex-col ${!temDescricao ? "mb-6 flex-grow" : "mb-3"}`}
                  >
                    <h3 className="font-bold text-xl text-gray-900 group-hover:text-brand-blue transition-colors leading-tight line-clamp-2">
                      {produto.titulo}
                    </h3>
                  ...
                  <div
                    className={`flex items-center mt-auto pt-5 border-t border-gray-100 ${temPreco ? "justify-between" : "justify-end"}`}
                  >
                    {temPreco && (
