...
const ContactForm: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await fetch("https://formsubmit.co/ajax/covitool@sapo.pt", {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        ...
        alert("Ocorreu um erro ao processar o seu pedido. Por favor, tente novamente ou contacte-nos por telefone.");
      }
    } catch (error) {
    ...
    <section className="max-w-4xl mx-auto px-4 py-1 relative" id="contacto">
      ...
      <div className="bg-white p-10 md:p-14 rounded-[40px] shadow-xl border border-gray-100 w-full relative z-10">
        ...
        <form className="space-y-6" onSubmit={handleSubmit}>
          ...
          <input type="hidden" name="_subject" value="Novo Contacto via Website Covitool!" />
          ...
          <div className="space-y-2">
            <label className="block text-[#153A81] text-xs font-bold ml-1 uppercase tracking-wider">Contacto Telefónico</label>
            <input 
              name="Telefone"
              className="w-full bg-gray-50 border border-transparent focus:border-[#B5D318] focus:bg-white transition-all py-4 px-6 rounded-2xl outline-none text-sm" 
              placeholder="+351 ..." 
          ...
          <div className="flex items-start gap-3 mt-4 px-1">
            ...
            />
            <label htmlFor="termos" className="text-sm text-gray-500 leading-relaxed cursor-pointer select-none">
              Compreendo e aceito que os dados fornecidos serão utilizados exclusivamente para responder a este contacto, em conformidade com a nossa <Link to="/politica-de-privacidade" className="text-[#153A81] font-bold hover:underline">Política de Privacidade</Link> e <Link to="/termos-e-condicoes" className="text-[#153A81] font-bold hover:underline">Termos e Condições</Link>.
            </label>
          </div>
          ...
          <div className="flex justify-center mt-8 pt-4">
            ...
            >
              {isSubmitting ? (
                <><span className="material-symbols-outlined animate-spin text-xl">refresh</span> A enviar...</>
              ) : (
                'Enviar Mensagem'
              )}
            </button>
      ...
      {showSuccessModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#153A81]/40 backdrop-blur-sm transition-all animate-in fade-in duration-300">
          <div className="bg-white rounded-[32px] p-8 md:p-12 max-w-lg w-full shadow-2xl flex flex-col items-center text-center transform scale-100 animate-in zoom-in-95 duration-300">
            ...
            {/* Ícone de Sucesso */}
            <div className="w-20 h-20 bg-[#B5D318]/20 rounded-full flex items-center justify-center mb-6">
              <span className="material-symbols-outlined text-5xl text-[#B5D318]">check_circle</span>
            </div>
