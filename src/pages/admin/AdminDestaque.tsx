...
export default function AdminDestaque() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const destaqueAntigo = editId
        ? destaques.find((d) => d.id === editId)
        : null;
  ...
  const handleDelete = async (id: string) => {
    if (
      window.confirm(
        "Apagar este destaque? A Capa e o PDF serão eliminados do servidor. O destaque anterior passará a ser o principal caso exista.",
      )
    ) {
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="flex flex-col gap-2">
              ...
              <input
                ...
                required
                className="p-3 border border-gray-300 rounded-xl focus:ring-brand-lime outline-none"
          ...
          <div className="flex flex-col gap-2">
            ...
            <textarea
              ...
              rows={3}
              className="p-3 border border-gray-300 rounded-xl focus:ring-brand-lime outline-none"
              placeholder="Descubra as promoções exclusivas deste mês..."
            ></textarea>
          </div>
          ...
          <div className="flex justify-end gap-3 mt-2">
            ...
            <button
              type="submit"
              disabled={loading}
              className={`${editId ? "bg-blue-600 hover:bg-blue-700 text-white" : "bg-brand-blue hover:bg-[#0d2657] text-brand-lime"} px-8 py-3.5 rounded-xl font-bold transition-all flex items-center gap-2 shadow-md disabled:opacity-50 disabled:cursor-not-allowed`}
            >
              {loading ? (
                <>
