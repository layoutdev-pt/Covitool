    ...
    try {
      ...
      if (editId) {
        ...
        const { error } = await supabase.from('produtos_destaque').insert([
          { titulo, marca, descricao, preco, imagem_url: urlImagem }
        ]);
        if (error) throw error;
      ...
      <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 transition-all duration-300 relative">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="flex flex-col gap-2">
              <label className="text-sm font-semibold text-gray-700">Marca da Peça</label>
              <input type="text" value={marca} onChange={(e) => setMarca(e.target.value)} className="p-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-[#B5D318] focus:border-[#153A81] outline-none transition-all" placeholder="Ex: BOSCH, BREMBO, etc." />
            </div>

            <div className="flex flex-col gap-2">
              <label className="text-sm font-semibold text-gray-700">Preço (com símbolo)</label>
              <input type="text" value={preco} onChange={(e) => setPreco(e.target.value)} className="p-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-[#B5D318] outline-none transition-all" placeholder="Ex: 489,00 € (Opcional)" />
            </div>

            <div className="flex flex-col gap-2">
              <label className="text-sm font-semibold text-gray-700">
                Imagem de Destaque {editId && <span className="text-gray-400 font-normal">(Opcional)</span>}
              </label>
              <input 
                type="file" 
                accept="image/*" 
          ...
          <div className="flex flex-col gap-2">
            ...
            <textarea value={descricao} onChange={(e) => setDescricao(e.target.value)} rows={2} className="p-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-[#B5D318] outline-none resize-none" placeholder="Breve descrição (Opcional)"></textarea>
          </div>
            ...
            <tbody>
              {produtos.map((produto) => (
                <tr key={produto.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                  ...
                  <td className="py-4 font-semibold text-gray-500">{produto.marca || '-'}</td>
                  <td className="py-4 font-semibold text-gray-700">{produto.preco || '-'}</td>
                  <td className="py-4 text-right pr-2 whitespace-nowrap">
                    {/* BOTÃO EDITAR */}
                    <button onClick={() => handleEdit(produto)} className="text-blue-500 hover:text-blue-700 hover:bg-blue-50 p-2 rounded-lg transition-colors border border-transparent hover:border-blue-100 mr-2" title="Editar">
              ...
              {produtos.length === 0 && (
                <tr>
                  <td colSpan={5} className="py-8 text-center text-gray-500 font-medium bg-gray-50 rounded-lg">Nenhum produto adicionado.</td>
                </tr>
              )}
            </tbody>
          </table>
