...
export default function AdminFolhetos() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const folhetoAntigo = editId
        ? folhetos.find((f) => f.id === editId)
        : null;
  ...
  const handleDelete = async (id: string) => {
    if (
      window.confirm(
        "Tem a certeza que deseja apagar este folheto? A Capa e o PDF serão eliminados permanentemente do servidor.",
      )
    ) {
                    ...
                    <div className="flex flex-col gap-1 text-xs">
                      {folheto.validade ? (
                        <span className="text-gray-500">
                          <b className="text-gray-700">Até:</b>{" "}
                          {formatarData(folheto.validade)}
                        </span>
                      ) : (
